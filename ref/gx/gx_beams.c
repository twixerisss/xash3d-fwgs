/*
gx_beams.c - beams rendering (Wii GX native port)
...
static float	rgNoise[NOISE_DIVISIONS+1];
...
static void R_BeamDraw( BEAM *pbeam, float frametime )
{
	...
	if( FBitSet( pbeam->flags, FBEAM_STARTENTITY|FBEAM_ENDENTITY ))
	{
		...
		if( pbeam->amplitude >= 0.50f )
			pbeam->segments = VectorLength( pbeam->delta ) * 0.25f + 3.0f;
		else pbeam->segments = VectorLength( pbeam->delta ) * 0.075f + 3.0f;
	}
...
{
	GX_SetZMode( GX_TRUE, GX_LEQUAL, fTrans ? GX_FALSE : GX_TRUE );
